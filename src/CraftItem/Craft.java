...
public class Craft extends PluginBase {
    ...
    public void onEnable() {
        instance = this;
        this.getLogger().info("自定义合成加载成功_");
        getJsonConfig();
        getFrameConfig();
        getCraftItemConfig();
        registerCraft();
    ...
    private void getJsonConfig(){
        if(!new File(this.getDataFolder()+CRAFT).exists()){
            this.saveResource("craft.json");
        }
        if(craft == null){
            craft = new Config(this.getDataFolder()+CRAFT,Config.JSON);
        }
    }

    private void getFrameConfig(){
        if(!new File(this.getDataFolder()+FRAME).exists()){
            this.saveResource("frame.json");
        }
        if(frame == null){
            frame = new Config(this.getDataFolder()+FRAME,Config.JSON);
        }
    }

    private Config getCraftItemConfig(){
        if(!new File(this.getDataFolder()+NBT_ITEM).exists()){
            this.saveResource("nbtItems.json");
        }
        if(nbt == null){
            nbt = new Config(this.getDataFolder()+NBT_ITEM,Config.JSON);
        }
        return nbt;
    }
